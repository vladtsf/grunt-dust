describe "grunt-dust", ->
	...
	before ( done ) ->
		...
		grunt.file.recurse dst, (abspath, rootdir, subdir, filename) =>
			...
			# console.log defs.ast = esprima.parse defs.raw
			parseBundle abspath

			structure["#{subdir}#{path.sep}#{filename}"] = defs#if filename is "views.js" then util._extend defs, req defs.raw else defs
	...
	# 		console.log @structure[ path.join "many-targets", "tags.js" ].returning
